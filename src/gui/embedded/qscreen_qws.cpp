...
/*!
    ...
    control registers. The initDevice() function can be reimplemented
    to set up the graphics card. Note, however, that connect() is
    called \e before the initDevice() function, so for some hardware,
    some initialization might have to be done in the connect()
    function that would normally be done in initDevice().

    Likewise, just before a \l{Qt for Embedded Linux} application exits, it
...
/*!
    ...
    This function is called by every \l{Qt for Embedded Linux} application on
    ...
    \snippet doc/src/snippets/code/src.gui.embedded.qscreen_qws.cpp 0

    For example, to use the mach64 driver on fb1 as display 2:

    \snippet doc/src/snippets/code/src.gui.embedded.qscreen_qws.cpp 1
...
/*!
    ...
    The \a displayId identifies the \l{Qt for Embedded Linux} server to connect
    to. The \a classId specifies the class identifier.
*/
