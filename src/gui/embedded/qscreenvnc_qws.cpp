...
{
    hwaccel = true;
    oldCursor = static_cast<QVNCCursor*>(qt_screencursor); // hw: XXX
    qt_screencursor = this;

    enable = oldCursor->isVisible();
    oldCursor->hide();

    set(oldCursor->cursor, oldCursor->hotspot.x(), oldCursor->hotspot.y());
}

QVNCClientCursor::~QVNCClientCursor()
{
    oldCursor->set(cursor, hotspot.x(), hotspot.y());
    if (enable)
        oldCursor->show();
    qt_screencursor = oldCursor;
}
...
bool QVNCScreen::initDevice()
{
    ...
    bool ok = QProxyScreen::initDevice();
