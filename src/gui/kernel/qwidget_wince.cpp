...
void QWidgetPrivate::create_sys(WId window, bool initializeWindow, bool destroyOldWindow) {
    ...
    if (window) {                                // override the old window
        ...
        static QRect defaultRect;
        if (defaultRect.isNull()) {
            const QRect& availableGeometry = qApp->desktop()->availableGeometry();
            const int w = qMin(qreal(availableGeometry.width()) * qreal(0.618f), qreal(100));
            const int h = qMin(qreal(availableGeometry.height()) * qreal(0.618f), qreal(100));
            const int x = (availableGeometry.width() - w) / 2;
            const int y = (availableGeometry.height() - h) / 2;
            defaultRect.setRect(x, y, w, h);
        }

        const bool wasMoved = q->testAttribute(Qt::WA_Moved);
        int x = wasMoved ? data.crect.left() : defaultRect.x();
        int y = wasMoved ? data.crect.top() : defaultRect.y();
        int w = defaultRect.width();
        int h = defaultRect.height();
        ...
        if (AdjustWindowRectEx(&rect, style, FALSE, exsty)) {
            ...
            if (q->testAttribute(Qt::WA_Resized)) {
                ...
                if (!wasMoved) {
                    x = qMax(x - (w - defaultRect.width()) / 2, 0);
                    y = qMax(y - (h - defaultRect.height()) / 2, 0);
                }
            }
        }
