                             ...
                             const QPoint &offset)
{
    ...
    if (!toFlush.isEmpty()) {
        ...
        if(opaque && QScreen::instance()->supportsBlitInClients()) {
            ...
            if(directRegion().intersected(toFlush) == toFlush) {
                ...
                QScreen::instance()->blit(image(), geometry().topLeft(), flushRegion);
                QScreen::instance()->setDirty(toFlush.boundingRect());
                needRepaint = false;
            }
            QWSDisplay::ungrab();
        }
...
const QRegion QWSSharedMemSurface::directRegion() const
{
    ...
    if(cmem->address() && *(uint *)cmem->address() == directRegionId())
