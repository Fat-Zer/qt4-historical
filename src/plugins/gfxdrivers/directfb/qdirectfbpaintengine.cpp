    ...
    : s(0)
{
    ...
    description.caps = DFBSurfaceCapabilities(description.caps | DSCAPS_VIDEOONLY);
    ...
    : surface(0), q(p)
{
    ...
    int cacheLimit = qgetenv("QT_DIRECTFB_IMAGECACHE").toInt();
    if (cacheLimit > 0)
                                            ...
                                            const QRectF &src)
{
    ...
    if (img) {
        ...
        if (cost <= imageCache.maxCost()) {
            ...
            imgSurface = img->surface();
            if (imgSurface)
                imageCache.insert(cost, img, cost);
        }
        if (!imgSurface) {
            DFBSurfaceDescription description;
            description = QDirectFBScreen::getSurfaceDescription(image);
            fb->CreateSurface(fb, &description, &imgSurface);
#ifndef QT_NO_DIRECTFB_PALETTE
                                     ...
                                     Qt::ImageConversionFlags flags)
{
    ...
    if (!d->simpleClip || d->matrixRotShear) {
...
void QDirectFBPaintEngine::drawTiledPixmap(const QRectF &r,
                                           ...
                                           const QPointF &sp)
{
    if (!d->simpleClip || d->matrixRotShear || !sp.isNull()) {
